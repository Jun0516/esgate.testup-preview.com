...
$require = array('都道府県','市区町村','建物名/部屋番号','所在階','専有面積','お名前','電話番号','メールアドレス');
...
内容を確認いたしまして、担当者よりご連絡を差し上げます。
ご記入いただいたメールアドレスに、
内容の控えを送信いたしますのでご確認ください。
...
<style type="text/css">
	#formWrap{
		padding: 10rem 0;
		background: url(../assets/img/bg.jpg) top center/cover repeat;
	}
	#formWrap .lead{
		font-size: 2.4rem;
		text-align: center;
	...
	#formWrap .type_submit{
		background: linear-gradient(to bottom, #2BB7B3, #165C5A);
		margin-left: 8rem;
	}
	#formWrap .type_return{
...
<div id="formWrap">
	<div class="cont">
		...
		<?php if($empty_flag == 1){ ?>
		<div align="center">
			<h4 class="lead bold">入力にエラーがあります。下記をご確認の上「戻る」ボタンにて修正をお願い致します。</h4>
			<?php echo $errm; ?><br /><br />
			<div class="btn_content">
				<input type="button" value=" 戻る " onclick="history.back()" class="type_btn type_return">
			</div>
		</div>
		<?php }else{ ?>
		<p class="lead bold">以下の内容で間違いがなければ、<br class="sp">「送信する」ボタンを押してください。</p>
		...
		<div class="form__wrap">
			<form action="<?php echo h($_SERVER['SCRIPT_NAME']); ?>" method="POST">
				...
				<div align="center">
					...
					?>
					<div class="btn_content">
						<input type="button" value="戻る" onclick="history.back()" class="type_btn type_return submit_btn">
						<input type="submit" value="送信する" class="type_btn type_submit submit_btn">
					</div>
				</div>
			</form>
		</div>
...
<body>
	<div id="formWrap" align="center">
		<?php if($empty_flag == 1){ ?>
		<h4 class="lead">入力にエラーがあります。下記をご確認の上「戻る」ボタンにて修正をお願い致します。</h4>
		<div style="color:red"><?php echo $errm; ?></div>
		<br /><br /><input class="link" type="button" value=" 前画面に戻る " onclick="history.back()">
	</div>
</body>
</html>
<?php }else{ ?>
	...
	<a href="<?php echo $site_top ;?>" class="link">トップページへ戻る&raquo;</a>
</div>
<?php copyright(); ?>
...
//確認画面無しの場合の表示、指定のページに移動する設定の場合、エラーチェックで問題が無ければ指定ページヘリダイレクト
	...
	if($empty_flag == 1){ ?>
<div align="center"><h4>入力にエラーがあります。下記をご確認の上「戻る」ボタンにて修正をお願い致します。</h4><div style="color:red"><?php echo $errm; ?></div><br /><br /><input class="link" type="button" value=" 前画面に戻る " onclick="history.back()"></div>
<?php
...
function mailToAdmin($arr,$subject,$mailFooterDsp,$mailSignature,$encode,$confirmDsp){
	...
	$adminBody .="\n"."▼お問い合わせ内容"."\n\n";
	$adminBody .="＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝\n\n";
	$adminBody .= "＜物件情報＞\n".postToMail(array_slice($arr, 0, 9));//POSTデータを関数からセット
	$adminBody .= "\n\n＜ご連絡先情報＞\n".postToMail(array_slice($arr, 9));//POSTデータを関数からセット
	$adminBody .="\n＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝\n";
